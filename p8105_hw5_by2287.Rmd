...
The dataset `lda` is tidy data that contains data from a longitudinal study on `r nrow(lda)` participants, including the subject ID, arm, and observations over time
...
```{r}
#create a df based off of ttest_results that only filters on those tests whose H0 were rejected (p-value < 0.05)
